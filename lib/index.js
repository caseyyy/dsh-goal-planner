...
var API_PREFIX = "/api/goal-planner";
function dshHome() {
  return process.env.DSH_HOME && process.env.DSH_HOME.trim() !== "" ? process.env.DSH_HOME.trim() : join(homedir(), ".dsh");
}
  ...
  ctx.inject(["webServer"], (host) => {
    ...
    host.effect(() => {
      ...
      route(`${API_PREFIX}/save`, async (request, response) => {
        ...
        body.updatedAt = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
        save(body);
      ...
      route(`${API_PREFIX}/toggle`, async (request, response) => {
        ...
        doc.updatedAt = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
        save(doc);
